...
// Helper to check env key availability
...
function App() {
  ...
  useEffect(() => {
    checkApiKey();
  }, [currentUser, manualApiKey]);

  const checkApiKey = async () => {
    try {
      // 1. Check Manual Key
      if (manualApiKey && manualApiKey.length > 20) {
        setHasApiKey(true);
        return;
      }
  ...
  const handleSelectKey = async () => {
    try {
      if (window.aistudio && window.aistudio.openSelectKey) {
        ...
        // Show manual input if AI Studio is not available
  ...
  const handleGenerate = async () => {
    ...
    if (!hasApiKey) {
      setError("Generation cancelled: API Key is required.");
      setShowKeyInput(true);
      return;
    }
    ...
    try {
      ...
      const videoBlob = await generateVeoVideo(
        ...
        aspectRatio,
        quality,
        manualApiKey || getEnvApiKey(), // Pass the key explicitly
        (msg) => {
      ...
      if (msg.includes("API Key") || msg.includes("403") || msg.includes("401")) {
         setHasApiKey(false);
         setShowKeyInput(true);
         msg = "API Key Invalid or Expired. Please enter a valid key.";
      }
      setError(msg);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
      ...
      <main className="flex-grow w-full py-8">
        {!currentUser && <Auth onLogin={handleLogin} />}
        ...
        {currentUser && currentView === 'admin' && (
           <div className="px-4">
             <AdminDashboard hasApiKey={hasApiKey} onConnectKey={handleSelectKey} />
           </div>
        )}
        ...
        {currentUser && currentView === 'generator' && (
          <div className="max-w-3xl mx-auto px-4 flex flex-col gap-6 animate-fade-in">
             ...
             {!hasApiKey && (
              <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 shadow-sm transition-all">
                 <h2 className="text-lg font-bold text-indigo-900 mb-2 text-center">Configure Service</h2>
                 
                 {showKeyInput ? (
                    <div className="max-w-md mx-auto mt-2">
                       <label className="block text-xs font-bold text-indigo-800 mb-1">Enter Google GenAI API Key</label>
                       <div className="flex gap-2">
                         <input 
                           type="password" 
                           value={manualApiKey}
                           onChange={(e) => setManualApiKey(e.target.value)}
                           className="flex-grow px-3 py-2 border border-indigo-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm"
                           placeholder="AIzaSy..."
                         />
                         <Button 
                           onClick={() => { if(manualApiKey) setHasApiKey(true); }} 
                       ...
                       <p className="text-[10px] text-indigo-600 mt-2">
                         Get a key from <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="underline font-bold">Google AI Studio</a>.
                       </p>
                    </div>
                 ) : (
                   <div className="text-center">
                      <p className="text-indigo-700 mb-4 text-sm">
                        Video generation requires a Google Cloud API key.
                      </p>

...
export const generateVeoVideo = async (
  ...
  // Use passed key, or try environment, or fail.
  // We removed the hardcoded fallback to allow user to provide a working key.
  const validApiKey = apiKey || process.env.API_KEY;

  if (!validApiKey || validApiKey.includes("PLACEHOLDER") || validApiKey.length < 10) {
     throw new Error("Invalid API Key. Please connect a valid Google GenAI API key.");
  }

  // 1. Initialize API with the valid key
  ...
  const downloadUrl = `${generatedVideo.video.uri}&key=${validApiKey}`;
